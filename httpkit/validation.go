...
func (ub untypedBinder) BindRequest(r *http.Request, route *MatchedRoute, consumer toolkit.Consumer) error {
...
func (v *validation) contentType() {
	if toolkit.CanHaveBody(v.request.Method) {
...
func (v *validation) responseFormat() {
	if str := v.context.ResponseFormat(v.request, v.route.Produces); str == "" {
		v.result = append(v.result, errors.InvalidResponseFormat(v.request.Header.Get(toolkit.HeaderAccept), v.route.Produces))
	}
}
