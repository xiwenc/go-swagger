...
type Responder interface {
	WriteResponse(http.ResponseWriter, toolkit.Producer)
}
...
func (r *routableUntypedAPI) ConsumersFor(mediaTypes []string) map[string]toolkit.Consumer {
	return r.api.ConsumersFor(mediaTypes)
}
func (r *routableUntypedAPI) ProducersFor(mediaTypes []string) map[string]toolkit.Producer {
	return r.api.ProducersFor(mediaTypes)
}
func (r *routableUntypedAPI) AuthenticatorsFor(schemes map[string]spec.SecurityScheme) map[string]toolkit.Authenticator {
...
func (c *Context) BindValidRequest(request *http.Request, route *MatchedRoute, binder RequestBinder) error {
	...
	if toolkit.CanHaveBody(request.Method) {
		ct, _, err := toolkit.ContentType(request.Header)
		if err != nil {
			res = append(res, err)
		} else {
	...
	if len(res) == 0 {
		if str := NegotiateContentType(request, route.Produces, ""); str == "" {
			res = append(res, errors.InvalidResponseFormat(request.Header.Get(toolkit.HeaderAccept), route.Produces))
		}
	}
...
func (c *Context) ContentType(request *http.Request) (string, string, *errors.ParseError) {
	...
	mt, cs, err := toolkit.ContentType(request.Header)
	if err != nil {
		return "", "", err
	}
...
func (c *Context) Respond(rw http.ResponseWriter, r *http.Request, produces []string, route *MatchedRoute, data interface{}) {
	...
	rw.Header().Set(toolkit.HeaderContentType, format)
	...
	if err, ok := data.(error); ok {
		if format == "" {
			rw.Header().Set(toolkit.HeaderContentType, toolkit.JSONMime)
		}
