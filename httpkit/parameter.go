...
func (p *untypedParamBinder) typeForSchema(tpe, format string, items *spec.Items) reflect.Type {
	...
	case "file":
		return reflect.TypeOf(&toolkit.File{}).Elem()
...
func (p *untypedParamBinder) readValue(values toolkit.Gettable, target reflect.Value) ([]string, bool, bool, error) {
...
func (p *untypedParamBinder) Bind(request *http.Request, routeParams RouteParams, consumer toolkit.Consumer, target reflect.Value) error {
	// fmt.Println("binding", p.name, "as", p.Type())
	switch p.parameter.In {
	case "query":
		data, custom, hasKey, err := p.readValue(toolkit.Values(request.URL.Query()), target)
		if err != nil {
			return err
		}
		if custom {
	...
	case "header":
		data, custom, hasKey, err := p.readValue(toolkit.Values(request.Header), target)
		if err != nil {
			return err
		}
		if custom {
	...
	case "formData":
		...
		mt, _, e := toolkit.ContentType(request.Header)
		if e != nil {
		...
		if p.parameter.Type == "file" {
			...
			target.Set(reflect.ValueOf(toolkit.File{Data: file, Header: header}))
			return nil
		}

		if request.MultipartForm != nil {
			data, custom, hasKey, err := p.readValue(toolkit.Values(request.MultipartForm.Value), target)
			if err != nil {
				return err
			}
			if custom {
		...
		data, custom, hasKey, err := p.readValue(toolkit.Values(request.PostForm), target)
		if err != nil {
			return err
		}
		if custom {
