...
func NewAPI(spec *spec.Document) *API {
	return &API{
		spec:            spec,
		DefaultProduces: toolkit.JSONMime,
		DefaultConsumes: toolkit.JSONMime,
		...
		authenticators: make(map[string]toolkit.Authenticator),
		operations:     make(map[string]map[string]toolkit.OperationHandler),
...
type API struct {
	...
	consumers       map[string]toolkit.Consumer
	producers       map[string]toolkit.Producer
	authenticators  map[string]toolkit.Authenticator
	operations      map[string]map[string]toolkit.OperationHandler
...
func (d *API) RegisterAuth(scheme string, handler toolkit.Authenticator) {
	if d.authenticators == nil {
		d.authenticators = make(map[string]toolkit.Authenticator)
	}
	d.authenticators[scheme] = handler
}

// RegisterConsumer registers a consumer for a media type.
func (d *API) RegisterConsumer(mediaType string, handler toolkit.Consumer) {
	if d.consumers == nil {
		d.consumers = map[string]toolkit.Consumer{toolkit.JSONMime: toolkit.JSONConsumer()}
	}
	d.consumers[strings.ToLower(mediaType)] = handler
}

// RegisterProducer registers a producer for a media type
func (d *API) RegisterProducer(mediaType string, handler toolkit.Producer) {
	if d.producers == nil {
		d.producers = map[string]toolkit.Producer{toolkit.JSONMime: toolkit.JSONProducer()}
	}
	d.producers[strings.ToLower(mediaType)] = handler
}

// RegisterOperation registers an operation handler for an operation name
func (d *API) RegisterOperation(method, path string, handler toolkit.OperationHandler) {
	if d.operations == nil {
		d.operations = make(map[string]map[string]toolkit.OperationHandler)
	}
	um := strings.ToUpper(method)
	if b, ok := d.operations[um]; !ok || b == nil {
		d.operations[um] = make(map[string]toolkit.OperationHandler)
	}
	d.operations[um][path] = handler
}

// OperationHandlerFor returns the operation handler for the specified id if it can be found
func (d *API) OperationHandlerFor(method, path string) (toolkit.OperationHandler, bool) {
	if d.operations == nil {
		return nil, false
	}
...
func (d *API) ConsumersFor(mediaTypes []string) map[string]toolkit.Consumer {
...
func (d *API) ProducersFor(mediaTypes []string) map[string]toolkit.Producer {
...
func (d *API) AuthenticatorsFor(schemes map[string]spec.SecurityScheme) map[string]toolkit.Authenticator {
